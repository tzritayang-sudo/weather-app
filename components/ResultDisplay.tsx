...
const getColorHex = (colorName: string): string => {
  ...
  // 黑白灰系
  if (n.includes('black') || n.includes('黑') || n.includes('深黑') || n.includes('墨黑')) return '#1a1a1a';
  if (n.includes('white') || n.includes('白') || n.includes('象牙白') || n.includes('米白')) return '#ffffff';
  if (n.includes('grey') || n.includes('gray') || n.includes('灰') || n.includes('鋼灰') || n.includes('冷灰')) return '#6b7280';
  if (n.includes('silver') || n.includes('銀')) return '#cbd5e1';
  if (n.includes('charcoal') || n.includes('炭') || n.includes('煙燻')) return '#374151';

  // 藍色系
  if (n.includes('navy') || n.includes('藏青') || n.includes('深藍')) return '#1e3a8a';
  if (n.includes('royal') || n.includes('寶石藍') || n.includes('寶藍') || n.includes('鈷藍')) return '#2563eb';
  if (n.includes('sky') || n.includes('天藍') || n.includes('湖水藍')) return '#38bdf8';
  if (n.includes('baby blue') || n.includes('嬰兒藍')) return '#bfdbfe';
  if (n.includes('teal') || n.includes('藍綠') || n.includes('青')) return '#14b8a6';
  if (n.includes('靛藍') || n.includes('藍紫')) return '#4f46e5';

  // 紅色系
  if (n.includes('burgundy') || n.includes('酒紅') || n.includes('莓紅')) return '#7f1d1d';
  if (n.includes('red') || n.includes('紅') || n.includes('正紅')) return '#ef4444';
  if (n.includes('pink') || n.includes('粉') || n.includes('冷粉')) return '#f472b6';
  if (n.includes('rose') || n.includes('玫瑰') || n.includes('洋紅') || n.includes('品紅')) return '#e11d48';
  if (n.includes('salmon') || n.includes('鮭魚')) return '#fb923c';

  // 大地色系
  if (n.includes('brown') || n.includes('褐') || n.includes('棕') || n.includes('暖棕') || n.includes('焦糖')) return '#78350f';
  ...
  if (n.includes('green') || n.includes('綠') || n.includes('草綠') || n.includes('蘋果綠')) return '#22c55e';

  // 黃橙紫
  if (n.includes('yellow') || n.includes('黃') || n.includes('檸檬黃')) return '#facc15';
  if (n.includes('mustard') || n.includes('芥末')) return '#eab308';
  if (n.includes('orange') || n.includes('橘') || n.includes('橙') || n.includes('南瓜橙') || n.includes('暖橘')) return '#f97316';
  if (n.includes('purple') || n.includes('紫') || n.includes('冷紫')) return '#a855f7';
    ...
    <div className="w-full max-w-md mx-auto space-y-6 pb-20 animate-fade-in-up">
      ...
      <div className={`rounded-[2rem] p-8 ${card}`}>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {[
            { label: '現在', value: `${data.weather.temperature}°`, cls: isDarkMode ? 'text-white' : 'text-gray-900' },
      ...
      <div className="grid grid-cols-2 gap-4">
        {displayItems.map((item: any, i: number) => {
          ...
          const circleBgDynamic = 
            !isDarkMode && isWhite ? 'bg-slate-700' :
