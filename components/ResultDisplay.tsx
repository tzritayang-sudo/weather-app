...
const ResultDisplay: React.FC<ResultDisplayProps> = ({ data }) => {
  ...
  const getStyleLabel = (index: number) => {
    switch(index) {
      case 0: return "✨ 標準推薦 (Base Look)";
      case 1: return "🔥 時尚變體 (Trendy)";
      case 2: return "🌿 另一種氛圍 (Alternative)";
  ...
  return (
    <div className="space-y-8 animate-fade-in pb-10">
      ...
      <div className="bg-white rounded-[2.5rem] shadow-xl shadow-blue-100/50 overflow-hidden relative border border-slate-50">
        ...
        <div className="relative z-10 p-8 flex flex-col md:flex-row items-center md:items-stretch justify-between gap-8">
          
          {/* Current Weather */}
          <div className="flex-1 flex flex-col items-center md:items-start text-center md:text-left space-y-2">
            <span className="text-slate-400 text-xs font-bold tracking-widest uppercase bg-slate-50 px-3 py-1 rounded-full border border-slate-100">Selected Time Weather</span>
            <h2 className="text-4xl md:text-5xl font-black text-slate-800 tracking-tight">
              {data.location}
            </h2>
            <p className="text-xl text-slate-500 font-medium">{data.weather.description}</p>
            
            <div className="mt-6 flex items-center gap-6">
               <div className="text-center">
                 <div className="text-3xl font-bold text-slate-800">{data.weather.temperature.split(' ')[0]}</div>
                 <div className="text-xs text-slate-400 font-medium mt-1">氣溫</div>
               </div>
               <div className="w-px h-10 bg-slate-200"></div>
               <div className="text-center">
                 <div className="text-3xl font-bold text-indigo-600">{data.weather.feelsLike || data.weather.temperature.split(' ')[0]}</div>
                 <div className="text-xs text-indigo-400 font-medium mt-1">體感</div>
               </div>
               <div className="w-px h-10 bg-slate-200"></div>
               <div className="text-center">
                 <div className="text-3xl font-bold text-slate-800">{data.weather.rainProb}</div>
                 <div className="text-xs text-slate-400 font-medium mt-1">降雨率</div>
               </div>
            </div>
          </div>

          <div className="flex-shrink-0 p-4 animate-blob">
            {WeatherIcon}
          </div>
        </div>

        {/* 3-Day Forecast Section (Inside the card) */}
        {data.weather.forecast && data.weather.forecast.length > 0 && (
           <div className="relative z-10 border-t border-slate-100 bg-slate-50/50 p-6">
      ...
      {data.generatedImages && data.generatedImages.length > 0 && (
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
             <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">AI Style Gallery (3 Variations)</h3>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {data.generatedImages.map((img, index) => (
              <div key={index} className="bg-white rounded-3xl p-3 shadow-xl border border-slate-100 overflow-hidden group">
                <div className="relative aspect-[3/4] w-full rounded-2xl overflow-hidden bg-slate-100">
                   ...
                   <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4 pt-10">
                      <div className="text-white text-xs font-bold shadow-sm">
                        {getStyleLabel(index)}
                      </div>
                   </div>
                </div>
      ...
      {data.outfit.colorPalette && data.outfit.colorPalette.length > 0 && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-100">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-sm font-bold text-slate-400 uppercase tracking-wider">Color Palette</h3>
            ...
            <div className="flex items-center gap-2">
              {data.outfit.colorPalette.map((color, idx) => (
                <div key={idx} className="group relative">
                  <div 
                    className="w-12 h-12 rounded-full shadow-md border-2 border-white transition-transform transform hover:scale-110 hover:z-10 cursor-pointer"
      ...
      {/* 4. Outfit Gallery Items (Matches Image 1) */}
      <div className="grid grid-cols-2 md:grid-cols-4 gap-5">
          {data.outfit.items.map((item, index) => (
            <div 
              key={index} 
              className="group bg-white rounded-[2rem] p-6 shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-50 hover:shadow-[0_8px_30px_rgb(0,0,0,0.08)] transition-all duration-300 flex flex-col items-center justify-between text-center h-full relative overflow-hidden"
            >
              ...
              <div className="w-24 h-24 mb-4 relative">
                 ...
                 <div className="relative z-10 w-full h-full p-4 transform group-hover:-translate-y-1 transition-transform duration-300">
                   {renderIcon(item.icon)}
                 </div>
              </div>
              <div className="w-full">
                <div className="mb-2">
                   <span className="inline-block px-3 py-1 bg-slate-100 text-slate-600 text-xs font-bold border border-slate-200 rounded-full">
                     {item.color}
                   </span>
                </div>
                <p className="font-bold text-slate-800 text-lg mb-2 leading-tight">{item.item}</p>
      ...
      {/* 5. Pro Tip Card */}
      {data.outfit.tips && (
        <div className="bg-slate-900 text-white rounded-3xl p-8 shadow-2xl shadow-slate-400/20 relative overflow-hidden">
          ...
          <div className="relative z-10 flex flex-col md:flex-row gap-6 items-start">
             ...
             <div>
               <h4 className="font-bold text-lg mb-2 text-yellow-300">Stylist Note</h4>
               <p className="text-slate-300 leading-relaxed font-light text-lg">{data.outfit.tips}</p>
             </div>
          </div>
        </div>
      )}
